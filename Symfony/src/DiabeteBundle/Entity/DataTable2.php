...
class DataTable2
{
    ...
    public function getCountAllResults()
    {
        ...
        $identifier = $this->metadata->getSingleIdentifierFieldName();
        return (int) $this->repository->createQueryBuilder($this->tableName)
            ->select('count(' . $this->tableName . '.' .  $identifier . ')')->getQuery()->getSingleScalarResult();
    }
    ...
    public function getCountFilteredResults()
    {
        ...
        $identifier = $this->metadata->getSingleIdentifierFieldName();
        $qb->select('count(' . $this->tableName . '.' . $identifier . ')');
